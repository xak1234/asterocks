...
async function refreshCache(){
  try{
    ...
    const validateMagSanitized = (mag) => {
      const num = parseFloat(mag);
      return (num >= 0 && num <= 30 && isFinite(num)) ? num : null;
    };
    
    cache.latestMag = validateMagSanitized(cobs?.latestMag) ?? validateMagSanitized(ts?.observedMag) ?? validateMagSanitized(ts?.predictedMag) ?? null;
    cache.observedMag = validateMagSanitized(ts?.observedMag) ?? null;
    cache.predictedMag = validateMagSanitized(ts?.predictedMag) ?? null;
    cache.distanceKm = validateNumber(ts?.distanceKm, 0, 1e12) ?? null;
    cache.source = sanitizeString((cobs?.latestMag ? 'COBS' : (ts?.observedMag ? 'TheSkyLive(observed)' : (ts?.predictedMag ? 'TheSkyLive(predicted)' : 'none'))), 50);
...
app.get('/api/distance', rateLimit, async (req,res)=>{
  try {
    ...
    const validatedDistance = validateNumber(result.distanceKm, 0, 1e12);
    res.json({ ok: true, distanceKm: validatedDistance, timestamp: new Date().toISOString() });
  } catch (e) {
    console.error('Distance endpoint error:', e.message);
    res.status(500).json({ ok: false, error: 'Failed to fetch distance' });
  }
});
